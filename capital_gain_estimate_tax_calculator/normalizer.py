...
from .models import ZERO, Lot, NormalizedReport, ReportError, SourceSummary, classify_tax_term, validate_reconciliation
...
class BrokerageNormalizer(Protocol):
    schema_name: str
    source_note: str

    def recognizes_filename(self, source: CsvSource) -> bool: ...
    def recognizes(self, source: CsvSource) -> bool: ...
    def normalize(self, source: CsvSource) -> list[Lot]: ...


def detect_schema(path: Path) -> str | None:
    """Identify a supported brokerage export from its filename or CSV headers."""
    source = CsvSource(path=path, source_file=path.name)
    normalizer = DEFAULT_BROKERAGE_NORMALIZER_REGISTRY.resolve(source)
    return normalizer.schema_name if normalizer else None
...
def _create_lot(
    *,
    ...
    return Lot(
        ...
        short_term_gain_loss_usd=short_term,
        long_term_gain_loss_usd=long_term,
        total_realized_gain_loss_usd=reported,
        disallowed_loss_usd=disallowed,
        ...
        for row_number, values in enumerate(reader, start=2):
            ...
            lots.append(
                _create_lot(
                    ...
                    quantity=parse_decimal(row.get("Quantity")),
                    acquired_date=parse_date(row.get("Date Acquired")),
                    sale_date=parse_date(row.get("Date Sold")),
                    proceeds=proceeds,
                    basis=basis,
                    short_term=short_term,
                    long_term=long_term,
...
class FunctionBrokerageNormalizer:
    """Adapter for a brokerage-specific mapping function."""
    ...
    def __init__(
        ...
        *,
        ...
        source_note: str,
        mapper: Callable[[Path, str], list[Lot]],
    ) -> None:
        self.schema_name = schema_name
        self.source_note = source_note
        self._filename_markers = tuple(marker.casefold() for marker in filename_markers)
        self._required_headers = required_headers
        self._mapper = mapper

    def recognizes(self, source: CsvSource) -> bool:
        if self.recognizes_filename(source):
            return True
        return any(
            self._required_headers <= {value.strip() for value in header_row} for header_row in source.header_rows()
        )

    def recognizes_filename(self, source: CsvSource) -> bool:
        filename = source.path.name.casefold()
        return any(marker in filename for marker in self._filename_markers)

    def normalize(self, source: CsvSource) -> list[Lot]:
        return self._mapper(source.path, source.source_file)


class BrokerageNormalizerRegistry:
    """Select a normalizer without coupling import orchestration to brokerages."""

    def __init__(self, normalizers: tuple[BrokerageNormalizer, ...]) -> None:
        self._normalizers = normalizers

    def resolve(self, source: CsvSource) -> BrokerageNormalizer | None:
        filename_match = next(
            (normalizer for normalizer in self._normalizers if normalizer.recognizes_filename(source)),
            None,
        )
        if filename_match:
            return filename_match
        return next((normalizer for normalizer in self._normalizers if normalizer.recognizes(source)), None)

    def for_schema(self, schema_name: str) -> BrokerageNormalizer | None:
        return next((normalizer for normalizer in self._normalizers if normalizer.schema_name == schema_name), None)


BROKERAGE_NORMALIZERS: tuple[BrokerageNormalizer, ...] = (
    FunctionBrokerageNormalizer(
        schema_name=CHASE_SCHEMA,
        filename_markers=("chase",),
        required_headers=CHASE_REQUIRED_HEADERS,
        source_note="Reported realized G/L may include a separately shown disallowed-loss adjustment.",
        mapper=normalize_chase,
    ),
    FunctionBrokerageNormalizer(
        schema_name=FIDELITY_SCHEMA,
        filename_markers=("fidelity",),
        required_headers=FIDELITY_REQUIRED_HEADERS,
        source_note="Fidelity export is informational; trailing disclaimers are treated as source metadata only.",
        mapper=normalize_fidelity,
    ),
    FunctionBrokerageNormalizer(
        schema_name=SCHWAB_SCHEMA,
        filename_markers=("schwab",),
        required_headers=SCHWAB_REQUIRED_HEADERS,
        source_note="Charles Schwab does not include acquisition dates in this export; the closed date is used as a workbook placeholder.",
        mapper=normalize_charles_schwab,
    ),
)
DEFAULT_BROKERAGE_NORMALIZER_REGISTRY = BrokerageNormalizerRegistry(BROKERAGE_NORMALIZERS)
...
def normalize_sources(input_dir: Path, requested_year: int | None = None) -> NormalizedReport:
    ...
    recognized: list[tuple[str, BrokerageNormalizer, list[Lot]]] = []
    ignored: list[str] = []
    all_lots: list[Lot] = []
    for path in sorted(input_dir.rglob("*.csv")):
        ...
        if any(part.lower() in OUTPUT_DIRECTORY_NAMES for part in relative_path.parts[:-1]):
            ...
            continue
        source = CsvSource(path=path, source_file=source_file)
        normalizer = DEFAULT_BROKERAGE_NORMALIZER_REGISTRY.resolve(source)
        if normalizer is None:
            ignored.append(source_file)
            continue
        lots = normalizer.normalize(source)
        all_lots.extend(lots)
        recognized.append((source_file, normalizer, lots))
    if not all_lots:
    ...
    if not selected:
        raise ReportError(
            f"No recognized realized gain/loss rows have sale year {report_year}. Available years: {', '.join(map(str, years))}."
        )
    summaries = tuple(
        SourceSummary(
            source_name=normalizer.schema_name,
            source_file=source_file,
            recognized_rows=len(file_lots),
            ...
            latest_sale=max((lot.sale_date for lot in included), default=None),
            notes=normalizer.source_note,
        )
        for source_file, normalizer, file_lots in recognized
    )
