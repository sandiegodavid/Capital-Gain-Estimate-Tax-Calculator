...
def _render_lot_row(lot: object) -> str:
    ...
    return (
        f"<tr><td>{_escape(account_or_brokerage)}</td><td>{lot.acquired_date:%Y-%m-%d}</td>"
        f"<td>{lot.sale_date:%Y-%m-%d}</td><td>{lot.quantity:,}</td>"
        f"<td>{_currency(lot.proceeds_usd)}</td><td>{_currency(lot.cost_basis_usd)}</td>"
        f"<td>{_escape(lot.tax_term)}</td><td class=\"{gain_loss_class}\">{_currency(gain_loss)}</td></tr>"
    )
